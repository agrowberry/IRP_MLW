...
class GeometryManipulation:
    ...
    def call_map(self, output=True):
        k = int(self.path_ppt * self.num_turns)
        profile_ppt = self.profile_ppt
        profile_matrix = np.reshape(np.transpose(self.coil_spiral), (k, profile_ppt, 3))
        grad_array = self.grad_array
        profile_array = np.transpose(self.main_spiral)
        mapped_matrix = self.map_points(profile_matrix, grad_array, profile_array, k, profile_ppt)
        if output:
            return np.reshape(mapped_matrix, (self.n, 3))
        else:
            self.point_array = np.reshape(mapped_matrix, (self.n, 3))

    @staticmethod
    @njit
    def map_points(profile_matrix, normal_array, trans_array, k, profile_ppt):
        """
        ...
        """
        mapped_matrix = np.empty((k, profile_ppt, 3))
        ...
        for index, path_point in enumerate(trans_array):
            ...
            # pcd.points = o3d.utility.Vector3dVector(unmapped_profile_loop)
            # o3d.visualization.draw_geometries([pcd])

            rot_angle_y = np.pi / 2 - np.angle((normal_vector[0] ** 2 + normal_vector[1] ** 2) ** 0.5 +
                                               (normal_vector[2] * 1j), deg=False)
            rot_angle_z = -np.angle(normal_vector[0] + normal_vector[1] * 1j, deg=False)
            ...
            mapped_array = unmapped_profile_loop @ r_y @ r_z + path_point

            mapped_matrix[index, :] = mapped_array
    ...
    def call_gen(self):
        ...
        profile_ppt = self.profile_ppt
        normal_array = self.generate_normals_from_source(point_array, profile_ppt)
        return normal_array

    @staticmethod
    @njit()
    def generate_normals_from_source(
        point_array, profile_ppt, normalise=True
    ):
        """
        ...
        normal_array = np.empty_like(point_array)
        length = point_array.shape[0]

        for index in range(length):
            vector = point_array[index]
            if index + 1 < length:
                lat_vector = point_array[index + 1] - vector
            else:
                lat_vector = vector - point_array[index - 1]
            if index + profile_ppt < length:
                long_vector = point_array[index + profile_ppt] - vector
            else:
                long_vector = vector - point_array[index - profile_ppt]

            normal_vector = np.cross(long_vector, lat_vector)
            ...
            normal_array[index] = normal_vector
    ...
    def make_coil(
        ...
        self.point_array = np.transpose(self.call_map())
        if generate_normals:
